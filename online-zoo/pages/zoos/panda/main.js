...
//смена видео
const mainVideo = document.querySelector(".other-cams-main");
const arrow = document.querySelectorAll(".other-cams__information");

function changeVideo(movie) {
  let rel = mainVideo.src;
  mainVideo.src = movie.src;
  movie.src = rel;
}

for (let i = 0; i < arrow.length; i++) {
  arrow[i].addEventListener("click", function (event) {
    const video = arrow[i].nextElementSibling;
    changeVideo(video);
  });
}
